...
namespace XIVComboVX {
	internal class PluginAddressResolver: BaseAddressResolver {
		...
		protected override void Setup64Bit(SigScanner scanner) {
			...
			this.IsActionIdReplaceable = scanner.ScanText("81 F9 ?? ?? ?? ?? 7F 39 81 F9 ?? ?? ?? ??");

			this.GetActionCooldown = scanner.ScanText("E8 ?? ?? ?? ?? 0F 57 FF 48 85 C0");

			PluginLog.Verbose("===== X I V C O M B O =====");
			PluginLog.Verbose($"GetIcon address   0x{this.GetAdjustedActionId.ToInt64():X}");
			PluginLog.Verbose($"IsIconReplaceable 0x{this.IsActionIdReplaceable.ToInt64():X}");
